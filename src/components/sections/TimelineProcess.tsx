...
export function TimelineProcess({ steps, className }: TimelineProcessProps) {
  return (
    <ol className={cn("relative space-y-10 md:space-y-14", className)}>
      <span
        aria-hidden
        className="absolute left-5 top-2 bottom-2 w-px bg-gradient-to-b from-secondary/30 via-secondary/15 to-transparent md:left-6"
      />
      {steps.map((step, i) => (
        <li key={i} className="relative grid grid-cols-[2.5rem_1fr] gap-5 md:grid-cols-[3rem_1fr] md:gap-8">
          <div className="relative z-10 flex h-10 w-10 items-center justify-center rounded-full border border-border-subtle bg-background text-foreground shadow-soft-sm md:h-12 md:w-12">
