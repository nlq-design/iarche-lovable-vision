...
interface ProtectedCockpitRouteProps {
  children: ReactNode;
}

type AccessDeniedReason = 'NO_ROLE' | 'MFA_NOT_ENABLED' | 'STEPUP_REQUIRED';

const ProtectedCockpitRoute = ({ children }: ProtectedCockpitRouteProps) => {
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="max-w-md w-full">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Vous n'avez pas les permissions nécessaires pour accéder au Cockpit Commercial.
            </CardDescription>
          </CardHeader>
  ...
  if (deniedReason === 'MFA_NOT_ENABLED') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="max-w-md w-full">
          <CardHeader className="text-center">
            <KeyRound className="h-12 w-12 text-warning mx-auto mb-2" />
            <CardTitle>Authentification à deux facteurs requise</CardTitle>
            <CardDescription>
              Vous devez activer la 2FA pour accéder au Cockpit Commercial. 
...
export default ProtectedCockpitRoute;
