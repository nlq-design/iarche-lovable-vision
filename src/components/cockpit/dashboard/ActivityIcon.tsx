...
const icons: Record<string, React.ReactNode> = {
  note_added: <FileText className="h-3.5 w-3.5 text-foreground" />,
  status_changed: <Activity className="h-3.5 w-3.5 text-accent-foreground" />,
  task_created: <CheckCircle2 className="h-3.5 w-3.5 text-foreground" />,
  email_sent: <Mail className="h-3.5 w-3.5 text-foreground" />,
  call_logged: <Phone className="h-3.5 w-3.5 text-foreground" />,
  meeting_scheduled: <Calendar className="h-3.5 w-3.5 text-foreground" />,
  ...
  note: <FileText className="h-3.5 w-3.5 text-foreground" />,
  ai_action: <Brain className="h-3.5 w-3.5 text-foreground" />,
  synthesis_generated: <Database className="h-3.5 w-3.5 text-foreground" />,
  email_draft_generated: <Mail className="h-3.5 w-3.5 text-foreground" />,
};

export function ActivityIcon({ type, isAI }: { type: string; isAI?: boolean }) {
  if (isAI && !icons[type]) return <span className="mt-0.5 flex-shrink-0"><Brain className="h-3.5 w-3.5 text-foreground/70" /></span>;
