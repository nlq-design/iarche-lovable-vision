...
const AdminSolutions = () => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Êtes-vous sûr de vouloir supprimer cette solution ?')) return;
  ...
  return (
    <AdminLayout>
      <Helmet>
        <title>Gestion des solutions - Admin IArche</title>
      ...
      <div className="min-h-screen px-6 py-12">
        <div className="container mx-auto max-w-6xl">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h1 className="text-3xl font-bold text-foreground">Gestion des solutions</h1>
              <p className="text-muted-foreground mt-1">
                5 solutions SaaS : Team 5 Connect, Lexia, Collaboria, Dialogue Plus, Datalia
              </p>
            </div>
            <NavLink to="/admin/solutions/new">
              <Button>
          ...
          <div className="space-y-4">
            {articles.length === 0 ? (
              <Card className="p-8 text-center">
                <p className="text-muted-foreground">Aucune solution créée</p>
              </Card>
            ) : (
                ...
                <Card key={article.id} className="p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <NavLink to={`/solutions/${article.slug}`}>
                        <Button variant="outline" size="icon">
                          <Eye className="h-4 w-4" />
                        </Button>
                      </NavLink>
                      <NavLink to={`/admin/solutions/${article.id}`}>
                        <Button variant="outline" size="icon">
                          <Edit className="h-4 w-4" />
                        </Button>
                      </NavLink>
                      <NavLink to={`/admin/solutions/${article.id}/history`}>
                        <Button variant="outline" size="icon">
