        ...
        <article className="max-w-[1000px] mx-auto px-6 py-4">
          ...
          <div className="flex gap-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              {/* Métadonnées livre blanc (après le contenu, uniquement pour livres blancs) */}
              {article.resource_type === 'livre-blanc' && (
                <div className="mt-8 p-6 bg-muted/30 border border-border rounded-xl space-y-4 animate-fadeIn">
                  <h3 className="text-lg font-semibold text-foreground">Détails du document</h3>
                  
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {article.nombre_pages && (
                      <div className="flex flex-col">
                        <span className="text-xs text-muted-foreground mb-1">Pages</span>
                        <span className="text-sm font-medium text-foreground">{article.nombre_pages} pages</span>
                      </div>
                    )}
                    
                    {article.format_fichier && (
                      <div className="flex flex-col">
                        <span className="text-xs text-muted-foreground mb-1">Format</span>
                        <span className="text-sm font-medium text-foreground uppercase">{article.format_fichier}</span>
                      </div>
                    )}
                    
                    {article.taille_fichier_bytes && (
                      <div className="flex flex-col">
                        <span className="text-xs text-muted-foreground mb-1">Taille</span>
                        <span className="text-sm font-medium text-foreground">
                          {(article.taille_fichier_bytes / 1024 / 1024).toFixed(2)} MB
                        </span>
                      </div>
                    )}
                    ...
                    {article.niveau && (
                      <div className="flex flex-col">
                        <span className="text-xs text-muted-foreground mb-1">Niveau</span>
                        <Badge variant="secondary" className="text-sm w-fit">
                          {article.niveau === 'debutant' && 'Débutant'}
                          {article.niveau === 'intermediaire' && 'Intermédiaire'}
                          {article.niveau === 'expert' && 'Expert'}
                        </Badge>
                      </div>
                    )}
                    
                    {article.version_document && (
                      <div className="flex flex-col">
                        <span className="text-xs text-muted-foreground mb-1">Version</span>
                        <span className="text-sm font-medium text-foreground">{article.version_document}</span>
                      </div>
                    )}
                  </div>
                  
                  {article.langues_disponibles && article.langues_disponibles.length > 0 && (
                    <div className="flex flex-col">
                      <span className="text-xs text-muted-foreground mb-2">Langues disponibles</span>
                      <div className="flex gap-2 flex-wrap">
                        {article.langues_disponibles.map((langue) => (
                          <Badge key={langue} variant="outline" className="text-sm">
                            {langue === 'fr' && '🇫🇷 Français'}
                            {langue === 'en' && '🇬🇧 Anglais'}
                            {langue === 'es' && '🇪🇸 Espagnol'}
                          </Badge>
                  ...
                  {article.thematiques && article.thematiques.length > 0 && (
                    <div className="flex flex-col">
                      <span className="text-xs text-muted-foreground mb-2">Thématiques</span>
                      <div className="flex gap-2 flex-wrap">
                        {article.thematiques.map((thematique) => (
                          <Badge key={thematique} variant="secondary" className="text-sm">
                            {thematique}
                          </Badge>
                        ))}
                      </div>
